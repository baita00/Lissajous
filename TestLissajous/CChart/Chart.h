...
namespace NsCChart{		
	...
	*/
	...
	public:
		...
		// 等高线图或云图状态有变化，需要重绘
		void		SetContourChanged(bool bChanged);
	...
	public:
		...
		// 设置虚窗口映射时是否缩小适应
		void		SetVirtualShrink(bool shrink);
		// 设置是否启用虚窗口绘图
		void		SetVirtualEnable(bool enable);
	...
	public:
		...
		// 设置坐标拖动模式
		void		SetRangeDragMode(bool drag);
		// 重置缩放范围
	...
	public:	
		...
		// 添加一个数据点，同时移除首个数据点，且保持曲线X范围不变
		int			CrowdPoint2D(double x, double y, int nDataIndex=0, int nPlotIndex=0);
		// 添加一个数据点，当曲线上数据点超过SetDataMaxPoints设定的值后，移除首个数据点，且保持曲线X范围不变
		int			AddPoint2DStable(double x, double y, int nDataIndex=0, int nPlotIndex=0);
		// 添加2维曲线的多个数据点
		// nDataIndex表示曲线的序号,
		// nDataIndex如果等于当前曲线的条数，则新增一条曲线，如果小于0或者大于当前曲线条数，则添加失败
	...
	public:
		// 设置场函数，用于等高线图、云图
		void		SetFieldFcn(double (*_pFieldFcn) (double, double, void *), void *pPara=0);
		// 设置简化场函数，用于等高线图、云图
		void		SetFieldFcn(double (*_pFieldFcn) (double, double));
	#ifndef _WIN64
		// 设置场函数，用于等高线图、云图
		void		SetFieldFcn(double (_stdcall *_pFieldFcn) (double, double, void *), void *pPara=0);
		// 设置简化场函数，用于等高线图、云图
		void		SetFieldFcn(double (_stdcall *_pFieldFcn) (double, double));
	#endif
		// 设置场函数参数
		void		SetFieldPara(void *pPara);
		
		// 获得场函数指针，用于等高线图、云图
		double		(*GetFieldFcn( ))( double, double, void * );
		// 添加等高线数据点
		void		AddContourPoint(double x, double y, double h);
		// 读入控制点数据
		void		ReadCtrlPoint(TCHAR *fileName);
		// 清除等高线数据点
	...
	public:
		...
		// 设置柱图的最大组数
		void		SetStemMaxGroups(int num);
		// 设置在柱顶显示数值
		void		SetShowValOnTop(bool show);
		// 设置柱图增量模式
		void		SetStemDeltaMode(bool bDelta);
		// 设置柱图叠加模式
		void		SetStemAddupMode(bool bAddup);
		// 改变柱图的值
		void		UpdateStemValue(double val, int nWhichStem, int nWhichSeries = 0);
	...
	public:
		...
		// 设置云图的内存图像尺寸
		void		SetContourSize(int nWidth, int nHeight);
		// 自动设置云图的内存图像尺寸
		void		SetAutoContourSize(bool bAuto);
		...
		// 设置云图、热图多色模式
		void		SetMapMultiColor(bool mc);
		// 添加云图、热图的颜色点
		void		AddMapMultiColorPoint(double value, COLORREF cr);
		...
		// 设置等高线图、云图的绘图范围与计算范围相同，如果为false，则绘图范围需用SetPlotRange另行设置
		void		SetContourSameRange(bool bSame);

	public:
		// 设置热图的宽度和高度
		void		SetHeatDimension(int width, int height);
		// 设置热图的数据。注意iRow是行序号，iCol是列序号，不是X坐标和Y坐标。
	...
	public:
		...
		// 在曲线起点显示数据标题
		void		SetDataTitleShow(bool bShow, int nDataIndex, int nPlotIndex=0);
		// 设置数据标题显示时的对齐方式, nAlign默认值为DT_CENTER|DT_BOTTOM
		void		SetDataTitleAlignment(int nAlign, int nDataIndex, int nPlotIndex=0);
		// 所有数据自动设置颜色
		...
		// 是否多色渐变模式，优先于双色渐变
		bool		IsDataMultiColorMode(int nDataIndex, int nPlotIndex=0);
		// 设置多色渐变模式
		void		SetDataMultiColorMode(bool bMul, int nDataIndex, int nPlotIndex=0);
		// 获得多色渐变模式下输入节点的方法，0：数据点比例0.0-1.0，1：X数据值，2：Y数据值
		int			GetDataMultiColorInputType(int nDataIndex, int nPlotIndex=0);
		// 设置多色渐变模式下输入节点的方法
		void		SetDataMultiColorInputType(int nType, int nDataIndex, int nPlotIndex=0);
		...
		// 设置数据虚拟化(保存在数据库中)
		void		SetDataVirtual(bool bVirt, int nDataIndex, int nPlotIndex=0);
	...
	public:
		...
		// 设置子图Y坐标轴标题竖排，对X轴设置无效
		void		SetAxisYTitleVertical(bool vertical, int location, int nPlotIndex);
		// 设置Y坐标轴标题竖排，对X轴设置无效
		void		SetAxisYTitleVertical(bool vertical, int location);
		// 设置子图坐标轴标题
		...
		// 设置子图时间坐标轴混合模式时，小数部分的显示格式
		void		SetAxisDecimalTimeFormat(TCHAR *format, int location, int nPlotIndex);
		// 设置时间坐标轴混合模式时，小数部分的显示格式
		void		SetAxisDecimalTimeFormat(TCHAR *format, int location);
		// 设置子图时间坐标轴混合模式时，小数部分如果为0是否不显示小数部分
		void		SetAxisOmitZeroDecimalTime(bool omit, int location, int nPlotIndex);
		// 设置时间坐标轴混合模式时，小数部分如果为0是否不显示小数部分
		void		SetAxisOmitZeroDecimalTime(bool omit, int location);
		// 设置子图时间坐标轴基准
		...
		// 设置坐标轴首标签替换
		void		SetReplaceFirstLabel(bool bReplace, int location);
		// 设置子图坐标轴首标签替换
		void		SetReplaceFirstLabel(bool bReplace, int location, int nPlotIndex);
		// 设置坐标轴首标签替换字符串
		void		SetAlternateFirstLabel(TCHAR* strLabel, int location);
		// 设置子图坐标轴首标签替换字符串
		void		SetAlternateFirstLabel(TCHAR* strLabel, int location, int nPlotIndex);
		// 设置坐标轴末标签替换
		void		SetReplaceLastLabel(bool bReplace, int location);
		// 设置子图坐标轴末标签替换
		void		SetReplaceLastLabel(bool bReplace, int location, int nPlotIndex);
		// 设置坐标轴末标签替换字符串
		void		SetAlternateLastLabel(TCHAR* strLabel, int location);
		// 设置子图坐标轴末标签替换字符串
		void		SetAlternateLastLabel(TCHAR* strLabel, int location, int nPlotIndex);
	...
	public:
		...
		// 设置图例显示复选框，此选项只有在图例显示水平为0时才有效
		void		SetLegendCheckBox(bool bShow, int nPlotIndex=0);
	...
	////////////////////////////////////////////////////////////////////////////////////////////////////
	////////////////////////////////////////////////////////////////////////////////////////////////////
	// Declaration of class MyMemDC
	
	// 内存DC类，用于某些情况下手动双缓冲绘图，和MFC中的CMemDC功能差不多
	class	CChart_API	MyMemDC
	{
	public:
		// MyMemDC内部所用的数据结构
		struct	stMyMemDCPara;
		// 获得stMyMemDCPara的大小
		static	int		GetParaSize();
		// 获得内部数据的指针
		stMyMemDCPara *GetPara();
	protected:
		// 内部数据
		stMyMemDCPara *m_pPara;
		// 释放内部资源
		void	Release();
